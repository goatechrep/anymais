...
export const Dashboard: React.FC<DashboardProps> = ({ lang, setLang, onLogout }) => {
  ...
  // Note: Simplification for this demo, resetting image on type change unless we flag it as custom
  useEffect(() => {
     // If the current image is one of the placeholders, update it. 
     // If it's a data URI (custom upload - typically starts with data:image), keep it, 
     // BUT since getPlaceholderImage returns http links, we can check for http.
     // If the user uploaded a custom image, it will be a data URI (base64).
  ...
  const checkPlanAccess = (view: DashboardView) => {
      ...
      // Basic: Only Profile & Adoption
      if (currentUser.plan === 'basic') return false;
      
      // Start: Basic + Health + Services
  ...
  const handleUpdatePlan = (newPlan: PlanType) => {
    ...
    // Note: We don't kick user out of the view instantly for smoother UX, but checkPlanAccess will be valid next render
  };
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      `}>
            ...
            {showPetDropdown && (
              <div className="absolute top-full left-0 w-full mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl overflow-hidden z-50 animate-fade-in ring-1 ring-black/5">
                  ...
                  {pets.map(pet => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (activeView === 'create-pet' || activeView === 'user-profile') setActiveView('profile');
                    ...
                    >
                      <div className={`relative rounded-full flex-shrink-0`}>
                        ...
                        {activePetId === pet.id && (
                            <div className="absolute -bottom-1 -right-1 bg-brand-600 text-white rounded-full p-[2px] border-2 border-white">
                ...
                <div className="p-1 border-t border-gray-100">
                  <button
                    onClick={() => {
                      setActiveView('create-pet');
                      setShowPetDropdown(false);
                      setActivePetId(null);
                      setMobileMenuOpen(false);
                    }}
                    className="w-full flex items-center gap-2 p-2 rounded-xl text-brand-600 hover:bg-brand-50 text-sm font-bold transition-colors"
                  >
        ...
        <nav className="flex-1 p-4 overflow-y-auto md:overflow-visible overflow-x-hidden">
          ...
          <div className="relative z-30">
            <NavItem 
                ...
                icon={Home} 
                label={t.dashProfile} 
                description={t.tooltipProfile}
                planReq={t.reqPlanBasic}
            />
            <NavItem 
                view="adoption" 
                icon={PawPrint} 
                label={t.dashAdoption} 
                description={t.tooltipAdoption}
                planReq={t.reqPlanBasic}
            />
            <NavItem 
                view="dating" 
                icon={Heart} 
                label={t.dashDating} 
                locked={!checkPlanAccess('dating')} 
                description={t.tooltipDating}
                planReq={t.reqPlanPremium}
            />
            <NavItem 
                view="health" 
                icon={Stethoscope} 
                label={t.dashHealth} 
                locked={!checkPlanAccess('health')} 
                description={t.tooltipHealth}
                planReq={t.reqPlanStart}
            />
            <NavItem 
                view="services" 
                icon={Calendar} 
                label={t.dashServices} 
                locked={!checkPlanAccess('services')} 
                description={t.tooltipServices}
                planReq={t.reqPlanStart}
            />
          </div>
        </nav>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50">
           ...
           <div className="flex justify-center gap-2 mb-3">
            {Object.values(Language).map((l) => (
              <button 
                key={l} 
                onClick={() => setLang(l)}
                className={`px-2 py-1 rounded text-xs font-bold uppercase border ${lang === l ? 'bg-brand-100 text-brand-900 border-brand-100' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'}`}
              >
                {l}
              </button>
            ))}
          ...
          <button 
            onClick={() => { setActiveView('user-profile'); setMobileMenuOpen(false); }}
            className={`w-full flex items-center gap-3 p-2 rounded-lg transition-colors hover:bg-white hover:shadow-sm ${activeView === 'user-profile' ? 'bg-white shadow-sm ring-1 ring-pink-200' : ''}`}
          >
      ...
      <div className="md:hidden fixed top-0 w-full bg-white z-10 border-b p-4 flex justify-between items-center">
         <button onClick={() => setMobileMenuOpen(true)} className="p-2 -ml-2 text-gray-600 hover:bg-gray-100 rounded-full">
             <Menu size={24} />
         </button>
         ...
         <button onClick={() => setShowPetDropdown(!showPetDropdown)} className="bg-gray-100 p-1 rounded-full relative">
            {activePet ? (
                <img src={activePet.image} className="w-8 h-8 rounded-full object-cover ring-2 ring-brand-500 ring-offset-1" />
            ) : (
                <div className="w-8 h-8 flex items-center justify-center"><Plus size={20} className="text-gray-500" /></div>
            )}
         </button>
      </div>
        ...
        <div className="md:hidden fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowPetDropdown(false)}>
            <div className="bg-white w-full max-w-xs rounded-2xl p-4 shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-2 max-h-[60vh] overflow-y-auto">
                     {pets.map(pet => (
                        <button key={pet.id} onClick={() => { setActivePetId(pet.id); setShowPetDropdown(false); }} 
                            className={`flex items-center gap-3 w-full p-3 rounded-xl border transition-all ${activePetId === pet.id ? 'bg-brand-50 border-pink-200 text-brand-900' : 'border-transparent hover:bg-gray-50'}`}
                        >
                     ...
                     <button onClick={() => { setActiveView('create-pet'); setShowPetDropdown(false); setActivePetId(null); }} className="flex items-center gap-3 w-full p-3 text-brand-600 font-bold border-t mt-2 pt-4 hover:bg-brand-50 rounded-xl transition-colors">
                         <div className="w-10 h-10 rounded-full bg-brand-100 flex items-center justify-center"><Plus size={20} /></div>
                         {t.addNewPet}
                     </button>
                </div>
            </div>
      ...
      <main className="flex-1 overflow-y-auto p-6 md:p-10 pt-20 md:pt-10 pb-24 md:pb-10">
        ...
        {activeView === 'create-pet' && (
           <div className="max-w-xl mx-auto bg-white rounded-2xl shadow-sm p-8 border border-gray-100">
                  ...
                  <div className="relative group">
                      ...
                      <label className="absolute bottom-0 right-0 bg-brand-600 text-white p-2 rounded-full cursor-pointer hover:bg-brand-700 shadow-lg transition-transform hover:scale-110 z-10">
                          ...
                          <input 
                            ...
                            accept="image/*" 
                            className="hidden" 
                            onChange={handleNewPetPhotoUpload}
                          />
                      </label>
                  </div>
              </div>
              <div className="space-y-4">
                 ...
                 <div>
                    ...
                    >
                        {Object.keys(t).filter(k => k.startsWith('type')).map(key => {
                             // Simplified logic to map translation keys back to values
                 ...
                 <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1">{t.breedLabel}</label>
                    <input type="text" value={newPet.breed} onChange={e => setNewPet({...newPet, breed: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none bg-white text-gray-900" />
                 </div>
                 
                 <div className="grid grid-cols-2 gap-4">
                     <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">{t.ageLabel}</label>
                        <input type="number" value={newPet.age} onChange={e => setNewPet({...newPet, age: parseInt(e.target.value) || 0})} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none bg-white text-gray-900" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-1">{t.weight}</label>
                        <input type="number" value={newPet.weight} onChange={e => setNewPet({...newPet, weight: parseFloat(e.target.value) || 0})} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none bg-white text-gray-900" />
                    </div>
                 </div>
                 ...
                 <div>
                    ...
                    <textarea 
                        ...
                        className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none h-24 bg-white text-gray-900 resize-none"
                 ...
                 <div className="pt-2">
                    <label className="flex items-center space-x-3 cursor-pointer group">
                        <div className="relative">
                            <input 
                                ...
                                checked={newPet.availableForDating}
                                onChange={(e) => {
                                    if (e.target.checked && currentUser.plan !== 'premium') {
                                        setDatingAlert(true);
                                        return;
                                    }
                                    setDatingAlert(false);
                                    setNewPet({...newPet, availableForDating: e.target.checked});
                                }}
                            />
                    ...
                    {datingAlert && (
                        <div className="mt-2 p-3 bg-red-50 border border-red-100 rounded-lg text-red-700 text-sm flex items-center animate-fade-in">
             ...
             <div className="max-w-2xl mx-auto space-y-6">
                            ...
                            <label className="block text-sm font-bold text-gray-500 mb-1">{t.phone}</label>
                             {isEditingUser ? (
                                <input 
                                    ...
                                    onChange={e => setEditedUser({...editedUser, phone: formatPhone(e.target.value)})}
                                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none bg-white text-gray-900"
                 ...
                 <div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100">
                    <div className="flex justify-between items-start">
                        <div>
                           <h3 className="text-lg font-bold text-gray-800 mb-2">{t.currentPlan}</h3>
                           <span className={`inline-block px-3 py-1 rounded-full text-sm font-bold border ${getPlanColor(currentUser.plan || 'basic')}`}>
                               {getPlanName(currentUser.plan || 'basic')}
                           </span>
                        </div>
                        <Button variant="outline" size="sm" onClick={() => setShowPlanModal(true)}>
                           <CreditCard size={16} className="mr-2" /> {t.upgradePlan}
                        </Button>
                    </div>
                    <div className="mt-4 pt-4 border-t border-gray-100 text-sm text-gray-500">
                        {t.planBenefits}: {getPlanDescription(currentUser.plan || 'basic')}
                    </div>
                 </div>
                 ...
                 <div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100">
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                         ...
                         <button onClick={() => setActiveView('create-pet')} className="flex items-center justify-center gap-2 p-4 rounded-xl border-2 border-dashed border-gray-300 text-gray-400 hover:border-brand-400 hover:text-brand-600 transition-all">
                             <Plus size={20} /> {t.addNewPet}
                         </button>
                     </div>
                 </div>
             </div>
          ...
          <div className="max-w-3xl mx-auto">
             <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8 border border-gray-100 relative animate-fade-in">
                ...
                <div className="absolute top-6 right-6 flex gap-2">
                  {!isEditingPet ? (
                    <Button variant="outline" size="sm" onClick={() => { setEditedPet(activePet); setIsEditingPet(true); }} className="flex items-center gap-2">
                      <Pencil size={16} /> {t.editProfile}
                    </Button>
                  ) : (
                    <div className="flex gap-2">
                      <Button variant="ghost" size="sm" onClick={() => { setIsEditingPet(false); setEditedPet(activePet); }} className="text-gray-500 hover:bg-gray-100">
                        <X size={20} />
                      </Button>
                      <Button variant="primary" size="sm" onClick={savePetChanges} className="flex items-center gap-2">
                ...
                <div className="flex flex-col md:flex-row gap-8 items-start mt-4">
                  ...
                  <div className="flex flex-col items-center space-y-4 relative">
                    <div className="relative">
                      <img 
                        src={isEditingPet ? editedPet.image : activePet.image} 
                        alt={activePet.name} 
                        className="w-40 h-40 rounded-full object-cover border-4 border-brand-100 shadow-sm bg-gray-100" 
                      />
                      {isEditingPet && (
                        <label className="absolute bottom-0 right-0 bg-brand-600 text-white p-2 rounded-full cursor-pointer hover:bg-brand-700 shadow-lg transition-transform hover:scale-110">
                          <Camera size={20} />
                          <input 
                            type="file" 
                            accept="image/*" 
                    ...
                    {isEditingPet && (
                       <p className="text-xs text-gray-500">{t.changePhoto}</p>
                    )}
                  </div>
                      ...
                      <>
                        ...
                        <div className="flex flex-wrap gap-4 text-gray-600 mb-6">
                          <span className="bg-gray-100 px-3 py-1 rounded-full text-sm">
                            {t.breedLabel}: <span className="font-semibold text-gray-800">{activePet.breed}</span>
                          </span>
                          <span className="bg-gray-100 px-3 py-1 rounded-full text-sm">
                            {t.ageLabel}: <span className="font-semibold text-gray-800">{activePet.age}</span>
                          </span>
                          <span className="bg-gray-100 px-3 py-1 rounded-full text-sm">
                            {t.weight}: <span className="font-semibold text-gray-800">{activePet.weight}kg</span>
                          </span>
                        </div>
                        ...
                        {/* Dating Status Indicator in View Mode */}
                        {activePet.availableForDating && (
                            <div className="mt-4 inline-flex items-center gap-2 px-3 py-1 bg-pink-50 text-pink-600 rounded-full text-sm font-bold border border-pink-100">
                                <Heart size={14} fill="currentColor" />
                                {t.availableForDatingLabel}
                            </div>
                        )}
                      </>
                    ) : (
                      <div className="space-y-4 w-full animate-fade-in">
                        <div>
                          ...
                          <input 
                            ...
                            className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none bg-white text-gray-900"
                          />
                        </div>
                        ...
                        <div>
                           <label className="block text-sm font-bold text-gray-700 mb-1">{t.petType}</label>
                            <select 
                              ...
                              onChange={e => setEditedPet({...editedPet, type: e.target.value as Pet['type']})}
                              className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none bg-white text-gray-900"
                            >
                                <option value="dog">{t.typeDog}</option>
                                <option value="cat">{t.typeCat}</option>
                                <option value="bird">{t.typeBird}</option>
                                <option value="other">{t.typeOther}</option>
                            </select>
                        </div>
                        
                        {/* Dating Checkbox Edit Mode */}
                         <div className="pt-2">
                            <label className="flex items-center space-x-3 cursor-pointer group">
                                <div className="relative">
                                    <input 
                                        type="checkbox" 
                                        className="peer sr-only"
                                        checked={editedPet.availableForDating || false}
                                        onChange={(e) => {
                                            if (e.target.checked && currentUser.plan !== 'premium') {
                                                setDatingAlert(true);
                                                return;
                                            ...
                                            setDatingAlert(false);
                                            setEditedPet({...editedPet, availableForDating: e.target.checked});
                                        }}
                                    />
                                    <div className="w-10 h-6 bg-gray-200 rounded-full peer peer-checked:bg-brand-600 transition-colors"></div>
                                    <div className="absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform peer-checked:translate-x-4"></div>
                        ...
                        <div>
                          ...
                          <textarea 
                            ...
                            className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 outline-none h-32 bg-white text-gray-900"
                          />
                        </div>
                        ...
                        <div className="pt-8 mt-8 border-t border-gray-100">
                          ...
                          <button 
                            ...
                            className="flex items-center gap-2 text-red-500 hover:text-red-700 hover:bg-red-50 px-4 py-2 rounded-lg transition-colors font-medium text-sm"
                          >
                            <Trash2 size={16} />
                            {t.deletePet}
                          </button>
              ...
              <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                <table className="w-full text-left">
                  <thead className="bg-gray-50 text-gray-600 uppercase text-xs font-semibold">
                    <tr>
                      <th className="px-6 py-3">{t.healthVaccine}</th>
                      <th className="px-6 py-3">{t.healthDate}</th>
                      <th className="px-6 py-3">{t.healthNextDue}</th>
                      <th className="px-6 py-3">{t.healthStatus}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {activePet.vaccines && activePet.vaccines.length > 0 ? (
                      ...
                      <tr key={v.id} className="hover:bg-gray-50">
                        ...
                        <tr>
                            <td colSpan={4} className="p-8 text-center text-gray-500">{t.healthNoRecords}</td>
                        </tr>
                    )}
                  </tbody>
                </table>
        ...
        {activeView === 'services' && activePet && (
          checkPlanAccess('services') ? (
             <ServiceBooking providers={MOCK_SERVICES} lang={lang} />
          ) : (
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-2 flex justify-around items-center z-10 pb-safe">
          <button 
            ...
            disabled={!activePet}
            className={`p-2 rounded-lg transition-colors flex flex-col items-center ${activeView === 'profile' ? 'text-brand-600 bg-brand-50' : 'text-gray-400 hover:bg-gray-50'}`}
          >
            <Home size={24} />
          </button>
          <button 
            onClick={() => setActiveView('adoption')} 
             disabled={!activePet}
            className={`p-2 rounded-lg transition-colors flex flex-col items-center ${activeView === 'adoption' ? 'text-brand-600 bg-brand-50' : 'text-gray-400 hover:bg-gray-50'}`}
          >
            <PawPrint size={24} />
          </button>
          <button 
              ...
              disabled={!activePet}
              className={`p-2 rounded-lg transition-colors flex flex-col items-center ${activeView === 'health' ? 'text-brand-600 bg-brand-50' : 'text-gray-400 hover:bg-gray-50'}`}
          >
              <div className="relative">
                  <Stethoscope size={24} />
                  {!checkPlanAccess('health') && <div className="absolute -top-1 -right-1 bg-gray-200 rounded-full p-[2px]"><Lock size={10} className="text-gray-500" /></div>}
              </div>
          </button>
          <button 
              onClick={() => setActiveView('services')} 
              disabled={!activePet}
              className={`p-2 rounded-lg transition-colors flex flex-col items-center ${activeView === 'services' ? 'text-brand-600 bg-brand-50' : 'text-gray-400 hover:bg-gray-50'}`}
          >
               <div className="relative">
                  <Calendar size={24} />
                  {!checkPlanAccess('services') && <div className="absolute -top-1 -right-1 bg-gray-200 rounded-full p-[2px]"><Lock size={10} className="text-gray-500" /></div>}
               </div>
          </button>
           <button 
            onClick={() => setActiveView('user-profile')} 
            className={`p-2 rounded-lg transition-colors flex flex-col items-center ${activeView === 'user-profile' ? 'text-brand-600 bg-brand-50' : 'text-gray-400 hover:bg-gray-50'}`}
          >
            <UserIcon size={24} />
          </button>
        </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden relative animate-fade-in max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-8">
                    <div className="text-center mb-8">
                        <h2 className="text-2xl font-bold text-gray-900">{t.managePlan}</h2>
                        <p className="text-gray-500 mt-1">{t.upgradePlan}</p>
                    </div>
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                         ...
                         <div className={`bg-white rounded-xl p-6 border-2 cursor-pointer hover:shadow-md transition-all flex flex-col ${currentUser.plan === 'basic' ? 'border-green-500 ring-2 ring-green-100' : 'border-gray-100 hover:border-brand-200'}`}>
                            ...
                            <Button 
                                variant={currentUser.plan === 'basic' ? 'outline' : 'primary'} 
                                className="w-full mt-6" 
                                onClick={() => handleUpdatePlan('basic')}
                                disabled={currentUser.plan === 'basic'}
                            >
                                {currentUser.plan === 'basic' ? 'Atual' : 'Selecionar'}
                            </Button>
                        </div>
                         ...
                         <div className={`bg-white rounded-xl p-6 border-2 cursor-pointer hover:shadow-md transition-all flex flex-col ${currentUser.plan === 'start' ? 'border-brand-500 ring-2 ring-brand-100' : 'border-gray-100 hover:border-brand-200'}`}>
                            <div className="flex justify-between items-start">
                                <h3 className="text-lg font-bold text-gray-900">{t.planStart}</h3>
                                <span className="bg-brand-100 text-brand-700 text-[10px] font-bold px-2 py-1 rounded-full">POPULAR</span>
                            </div>
                            ...
                            <Button 
                                variant={currentUser.plan === 'start' ? 'outline' : 'primary'} 
                                className="w-full mt-6" 
                                onClick={() => handleUpdatePlan('start')}
                                disabled={currentUser.plan === 'start'}
                            >
                                {currentUser.plan === 'start' ? 'Atual' : 'Selecionar'}
                            </Button>
                        </div>
                         ...
                         <div className={`bg-white rounded-xl p-6 border-2 cursor-pointer hover:shadow-md transition-all flex flex-col ${currentUser.plan === 'premium' ? 'border-purple-500 ring-2 ring-purple-100' : 'border-gray-100 hover:border-purple-200'}`}>
                            ...
                            <Button 
                                variant={currentUser.plan === 'premium' ? 'outline' : 'primary'} 
                                className={`w-full mt-6 ${currentUser.plan !== 'premium' ? '!bg-purple-600 hover:!bg-purple-700' : '!text-purple-600 !border-purple-600 hover:!bg-purple-50'}`}
                                onClick={() => handleUpdatePlan('premium')}
                                disabled={currentUser.plan === 'premium'}
                            >
                                {currentUser.plan === 'premium' ? 'Atual' : 'Selecionar'}
                            </Button>
                        </div>
                    </div>
                </div>
